...
Sie definieren Regeln im Kontext eines Klassifizierungssatzes, was bedeutet, dass Regeln (wenn aktiviert) auf alle Report Suite- und Schlüssel-Dimensionskombinationen angewendet werden, die für den Klassifizierungssatz abonniert wurden. Diese Implementierung unterscheidet sich etwas von der Funktionsweise des alten Classification Rule Builders. Im Classification Rule Builder definieren Sie eine oder mehrere Regeln als Teil eines Regelsatzes separat und verknüpfen den Regelsatz dann mit einer oder mehreren Report Suites. In der neuen Benutzeroberfläche werden die Regeln im Klassifizierungssatz auch als Regelsatz bezeichnet, sind aber in derselben Benutzeroberfläche definiert, in der Sie andere Klassifizierungssatzattribute konfigurieren.
...
## Benutzeroberfläche für Regelsätze

Beim Erstellen oder Bearbeiten von Regeln verwenden Sie die Benutzeroberfläche des Regelsatzes.
...
Sie definieren jede einzelne Regel innerhalb des Regelsatzes in der Benutzeroberfläche „Regel“. Die -Schnittstelle besteht aus den folgenden Elementen:
...
| 2 | Die Eingabe für die ausgewählte Funktion. Die Eingabe hängt von der gewählten Funktion ab. Für die Funktion Regulärer Ausdruck ist die Eingabe beispielsweise ein regulärer Ausdruck und für die Funktion Aufspaltung ist die Eingabe ein Token. Geben Sie die entsprechende Eingabe für die spezifische Funktion ein. Beispiel: `^(.+)\:(.+)\:(.+)$` für einen regulären Ausdruck, der drei Klassifizierungen in einem internen Kampagnen-Code identifiziert. |
...
Legt eine Klassifizierung basierend auf einem bestimmten Wert fest, den die Schlüsseldimension enthält.
...
Legt eine Klassifizierung anhand eines bestimmten Werts fest, mit dem die Schlüsseldimension übereinstimmt.

+++ Details 

#### Erforderliche Eingabe

Geben Sie einen Wert für **[!UICONTROL Übereinstimmung]** ein. Beispiel: `em:FY2025:Summer`.
...
Wenn ein Schlüssel-Dimensionswert mehreren Regeln zugeordnet ist und die Regelsätze Regeln mit demselben Set-Klassifizierungsvorgang enthalten, bestimmt die letzte Regel den Wert für die Klassifizierung. Daher sollten Sie den wichtigsten Klassifizierungsvorgang für Sets als Teil der letzten Regel in Ihrem Regelsatz einstufen.

Wenn Sie mehrere Regeln erstellen, die nicht denselben Set-Klassifizierungsvorgang gemeinsam haben, spielt die Verarbeitungsreihenfolge keine Rolle.
