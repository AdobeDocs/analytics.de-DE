...
Siehe ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [Konfigurieren allgemeiner Kontoeinstellungen](https://experienceleague.adobe.com/de/docs/analytics-learn/tutorials/administration/manage-report-suites/configuring-general-account-settings){target="_blank"} für ein Demovideo.
...
| Zeitzone | Dies wirkt sich auf das Datum und die Uhrzeit aus, die mit Ihren Berichtdaten verbunden werden. Wenn die ausgewählte Zeitzone die Sommerzeit einhält, werden die stündlichen Daten automatisch angepasst, um dies widerzuspiegeln. Durch das Ändern der Zeitzone bei einer Live-Report Suite ergibt sich entweder eine Spitze oder Lücke in den Berichtdaten. Adobe empfiehlt, diese Änderung außerhalb der Spitzenzeiten vorzunehmen, um die Auswirkungen zu minimieren. |
...
| Organisations-ID | Die ID, die mit Ihrer bereitgestellten CX Enterprise-Firma verknüpft ist. Diese ID besteht aus einer 24-stelligen alphanumerischen Zeichenfolge gefolgt von @AdobeOrg (erforderlich). |
